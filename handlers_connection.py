"""Connection lifecycle for Tally Connector."""
...
@chat.function(
    "connect_tally",
    "Connect Tally account via credentials.",
    action_type="write",
    chain_callable=True,
    event="tally-connector.connect_tally",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_tally(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Tally Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Tally Account",
...
@chat.function(
    "disconnect_tally",
    "Disconnect Tally account.",
    action_type="write",
    chain_callable=True,
    event="tally-connector.disconnect_tally",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_tally(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
